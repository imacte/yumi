...
fn compile_rust_bpf(manifest_dir: &Path, package_name: &str, out_dir: &Path) -> std::path::PathBuf {
    ...
    let status = Command::new("cargo")
        ...
        .status()
        .unwrap_or_else(|_| panic!("无法执行 cargo build 编译 {}", package_name));

    if !status.success() {
        panic!("Rust eBPF 编译失败: {}!", package_name);
    }
    ...
    std::fs::copy(&built_obj, &bpf_obj)
        .unwrap_or_else(|_| panic!("无法复制 eBPF 产物: {} -> {}", built_obj.display(), bpf_obj.display()));
